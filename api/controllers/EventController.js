...
module.exports = {
  ...
  search: async function (req, res) {
    ...
    if (dateFrom) whereClause.date = {
      '>=': new Date(dateFrom).getTime()
    }
    if (dateTo) whereClause.date = {
      '>=': new Date(dateTo).getTime()
    }
    if (req.query.name) whereClause.name = {
      contains: req.query.name
    };
    if (req.query.organizer) whereClause.organizer = {
      contains: req.query.organizer
    };
    if (req.query.venue) whereClause.venue = {
      contains: req.query.venue
    };
  ...
  home: async function (req, res) {
    ...
    return res.view('event/home', {
      events: highlighted
    });
  },
  populate: async function (req, res) {

    var event = await Event.findOne(req.params.id).populate("consultants");
  ...
  aginate: async function (req, res) {
    if (req.wantsJSON) {
      ...
      return res.json(someEvents);

    } else {

      var count = await Event.count();

      return res.view('event/aginate', {
        total: count
      });
    }
